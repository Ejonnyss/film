#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Собирает PLYAZH_ARCANE_COLAB.ipynb — ноутбук для рендера фильма на free GPU."""

import json, os

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "PLYAZH_ARCANE_COLAB.ipynb")
...
cells.append(md("## 1. Проверка GPU"))
cells.append(code(
"!nvidia-smi\n",
"import torch, sys\n",
...
cells.append(md("## 2. Google Drive (сохранение прогресса)"))
cells.append(code(
...
cells.append(md("## 7. Запуск сервера ComfyUI"))
cells.append(code(
...
cells.append(md("## 8. Клиент ComfyUI API"))
cells.append(code(
...
"**ТОЧКА 1:** после явного разрешения генерируются SH001–SH006 и отдельный\n",
"SH003_TRIGGER с тем же seed, но с `arcane style` строго в конце промпта.\n",
"До разрешения оставьте `RUN_GATE_1_TESTS = False`. После просмотра семи кадров\n",
"и отдельного одобрения переключите `GATE_1_APPROVED = True` для остальных планов."
))
cells.append(code(
...
"RUN_GATE_1_TESTS = False  # True только после явного ok на повторный рендер семи тестов\n",
"GATE_1_APPROVED = False  # True только после одобрения 7 изображений Gate 1\n",
"KEYFRAME_SHOTS = SHOTS if GATE_1_APPROVED else SHOTS[:6]\n",
"PROMPT_CACHE_PATH = f\"{DIRS['keyframes']}/_prompt_cache.json\"\n",
"try:\n",
"    with open(PROMPT_CACHE_PATH, encoding='utf-8') as f: PROMPT_CACHE=json.load(f)\n",
"except (FileNotFoundError, json.JSONDecodeError):\n",
"    PROMPT_CACHE={}\n",
"def shot_render_signature(s, prompt=None):\n",
"    payload={'prompt':prompt or s['keyframe_prompt'],'negative':s['keyframe_negative'],\n",
"             'seed':s['seed'],'lora':ARCANE_LORA,'lora_weight':ARCANE_WEIGHT}\n",
"    return hashlib.sha256(json.dumps(payload,sort_keys=True,ensure_ascii=False).encode()).hexdigest()\n",
"def save_prompt_cache():\n",
"    tmp=PROMPT_CACHE_PATH+'.tmp'\n",
"    with open(tmp,'w',encoding='utf-8') as f: json.dump(PROMPT_CACHE,f,ensure_ascii=False,indent=2)\n",
"    os.replace(tmp,PROMPT_CACHE_PATH)\n",
"if not GATE_1_APPROVED and not RUN_GATE_1_TESTS:\n",
"    raise RuntimeError('СТОП: семь тестов Gate 1 нельзя генерировать до явного ok заказчика')\n",
"done = skipped = 0\n",
"for s in KEYFRAME_SHOTS:\n",
"    dst = f\"{DIRS['keyframes']}/{s['shot_id']}.png\"\n",
"    signature=shot_render_signature(s)\n",
"    if os.path.exists(dst) and PROMPT_CACHE.get(s['shot_id'])==signature:\n",
"        skipped += 1; continue\n",
"    if os.path.exists(dst):\n",
"        print(f\"{s['shot_id']}: промпт/LoRA изменились — сбрасываю только этот кадр и зависимые файлы\")\n",
"        os.unlink(dst)\n",
"        for key in ('faces','upscaled','clips'):\n",
"            ext='mp4' if key=='clips' else 'png'; p=f\"{DIRS[key]}/{s['shot_id']}.{ext}\"\n",
"            if os.path.exists(p): os.unlink(p)\n",
"    try:\n",
"        files_out = queue(wf_keyframe(s['keyframe_prompt'], s['keyframe_negative'], s['seed']))\n",
"        shutil.copy(files_out[0], dst)\n",
"        PROMPT_CACHE[s['shot_id']]=signature; save_prompt_cache()\n",
"        done += 1\n",
"        print(f\"[{done+skipped}/{len(KEYFRAME_SHOTS)}] {s['shot_id']} OK\")\n",
"    except Exception as e:\n",
"        raise RuntimeError(f\"{s['shot_id']} keyframe failed: {e}\") from e\n",
"\n",
"# A/B-тест LoRA trigger: тот же SH003, тот же seed и negative; меняется только хвост positive.\n",
"if not GATE_1_APPROVED:\n",
"    trigger_shot=next(s for s in SHOTS if s['shot_id']=='S01_SH003')\n",
"    trigger_id='S01_SH003_TRIGGER'\n",
"    trigger_prompt=trigger_shot['keyframe_prompt'] + ', arcane style'\n",
"    trigger_dst=f\"{DIRS['keyframes']}/{trigger_id}.png\"\n",
"    trigger_signature=shot_render_signature(trigger_shot, trigger_prompt)\n",
"    if not (os.path.exists(trigger_dst) and PROMPT_CACHE.get(trigger_id)==trigger_signature):\n",
"        if os.path.exists(trigger_dst): os.unlink(trigger_dst)\n",
"        trigger_out=queue(wf_keyframe(trigger_prompt, trigger_shot['keyframe_negative'], trigger_shot['seed']))\n",
"        shutil.copy(trigger_out[0],trigger_dst)\n",
"        PROMPT_CACHE[trigger_id]=trigger_signature; save_prompt_cache()\n",
"        print(trigger_id, 'OK — same seed, trigger appended only at prompt end')\n",
"    else:\n",
"        skipped += 1\n",
"\n",
"print(f'\\nГотово: {done} новых, {skipped} уже было')\n",
"if not GATE_1_APPROVED:\n",
"    print('СТОП — ТОЧКА 1. Покажите 7 кадров и ждите решения по trigger перед 59 планами.')"
))
...
"if not GATE_1_APPROVED:\n",
"    trigger_path=f\"{DIRS['keyframes']}/S01_SH003_TRIGGER.png\"\n",
"    if os.path.exists(trigger_path): paths.append(trigger_path)\n",
...
cells.append(md("### 14b. Синтез реплик"))
cells.append(code(
...
"через `LoraLoader`; базовый `ARCANE_WEIGHT` равен 0.50. Для стоп-гейта держите\n",
"его в согласованном диапазоне 0.45–0.60; текстовый trigger в промпт не добавляйте."
))
